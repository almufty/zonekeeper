...
// LOW-3 fix: paginate through all results instead of stopping at per_page
async function cfFetchAll(buildUrl, account, perPage = 100) {
  const results = [];
  let page = 1;
  while (true) {
    const url = `${buildUrl}&per_page=${perPage}&page=${page}`;
...
export async function listDnsRecords(account, zoneIdentifier) {
  assertCfId(zoneIdentifier, 'zone_identifier');
  return cfFetchAll(
    `${CF_BASE}/zones/${zoneIdentifier}/dns_records?type=A`,
    account,
  );
}

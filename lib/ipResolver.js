...
export async function resolvePublicIpV4() {
  ...
  // Fallback 4: ident.me
  try {
    const res = await fetch('https://v4.ident.me', { signal: AbortSignal.timeout(5000) });
    const ip = (await res.text()).trim();
    if (IPV4_RE.test(ip)) return ip;
  } catch (err) {
    logger.warn({ event: 'ipResolver.v4.identme.fail', err: err.message }, 'IPv4 ident.me failed');
  }
...
export async function resolvePublicIpV6() {
  ...
  try {
    ...
    logger.warn({ event: 'ipResolver.v6.identme.fail', err: err.message }, 'IPv6 ident.me failed');
  }
